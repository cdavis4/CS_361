...
{	
    ...
    for (i = 0; i < v->size; i++)
    {
       newData[i] = v->data[i];//copy data from old array into new array.
    }
...
void addDynArr(DynArr *v, TYPE val)
{
	...
	//	printf("capacity is bigger %d %d\n", v->size, v->capacity);
		_dynArrSetCapacity(v, 2 * v->capacity);
	}
	v->data[v->size] = val;
	//printf("Normal Dyn not called always %d %d, %d\n", v->size, v->capacity, v->data[v->size]);
	v->size = v->size + 1;
...
void swapDynArr(DynArr *v, int i, int  j)
{
	...
	assert(v != 0);
	assert(v->size);
	assert(j < sizeDynArr(v) && (j >=0 && j > i));
	TYPE iValue;
	TYPE jValue;
	iValue = getDynArr(v, i);
	jValue = getDynArr(v, j);

	putDynArr(v, i, jValue);
	putDynArr(v, j, iValue);
